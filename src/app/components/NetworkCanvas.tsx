...
const NODES = {
  // Sources (col 1)
  ssh:        { rx: 0.06, ry: 0.16, r: 9,  label: 'SSH Auth',     sublabel: 'Log Source',   color: '#4DABF7', type: 'source' as const },
  ueba:       { rx: 0.06, ry: 0.38, r: 9,  label: 'UEBA',          sublabel: 'Behavioral',   color: '#9775FA', type: 'source' as const },
  network:    { rx: 0.06, ry: 0.60, r: 9,  label: 'Network',       sublabel: 'Packets',      color: '#51CF66', type: 'source' as const },
  hdfs:       { rx: 0.06, ry: 0.82, r: 9,  label: 'HDFS',          sublabel: 'System',       color: '#D29922', type: 'source' as const },
  // ML Models (col 2)
  bilstm:     { rx: 0.24, ry: 0.16, r: 14, label: 'Bi-LSTM',       sublabel: 'SSH Model',    color: '#4DABF7', type: 'model'  as const },
  cnnlstm:    { rx: 0.24, ry: 0.38, r: 14, label: 'CNN-BiLSTM',    sublabel: '8-Head Attn',  color: '#9775FA', type: 'model'  as const },
  lgbm:       { rx: 0.24, ry: 0.60, r: 14, label: 'LightGBM',      sublabel: 'Stage 1',      color: '#51CF66', type: 'model'  as const },
  isolation:  { rx: 0.24, ry: 0.82, r: 14, label: 'IsoForest',     sublabel: 'Anomaly',      color: '#D29922', type: 'model'  as const },
  // Network extended pipeline (col 3 & 4)
  xgb:        { rx: 0.43, ry: 0.60, r: 14, label: 'XGBoost',       sublabel: '21-class',     color: '#F59E0B', type: 'model'  as const },
  pytorch:    { rx: 0.60, ry: 0.60, r: 14, label: 'PyTorch AE',    sublabel: 'Autoencoder',  color: '#EC4899', type: 'model'  as const },
...
const SOURCE_PATHS: Record<'ssh' | 'ueba' | 'network' | 'hdfs', NodeKey[]> = {
  ssh:     ['bilstm', 'agent'],
  ueba:    ['cnnlstm', 'agent'],
  network: ['lgbm', 'xgb', 'pytorch', 'agent'],
  hdfs:    ['isolation', 'agent'],
};

const SOURCE_MAP: Record<string, 'ssh' | 'ueba' | 'network' | 'hdfs'> = {
  SSH: 'ssh', UEBA: 'ueba', Network: 'network', HDFS: 'hdfs', system: 'hdfs'
};
...
function resolveOutput(isAttack: boolean, verdict?: string, attackType?: string, source?: string): OutputKey {
  ...
  if (source === 'HDFS' || source === 'system') return 'ddos';
  return 'ddos';
}
...
const NODE_INFO: Record<string, { title: string; desc: string; badge: string; badgeColor: string }> = {
  ...
  hdfs:       { title: 'HDFS Logs',                 desc: 'Hadoop Distributed File System log activity monitoring',                                                       badge: 'Source',        badgeColor: '#D29922' },
  ...
  isolation:  { title: 'Isolation Forest',          desc: 'Unsupervised anomaly detection for structural sequence deviations in HDFS logs.',                                badge: 'Unsupervised',  badgeColor: '#D29922' },
...
interface Particle {
  id: string;
  output: OutputKey;
  sourceKey: 'ssh' | 'ueba' | 'network' | 'hdfs';
  path: NodeKey[];
  segIdx: number;
...
function buildPath(srcKey: 'ssh' | 'ueba' | 'network' | 'hdfs', output: OutputKey): NodeKey[] {
  return [srcKey, ...SOURCE_PATHS[srcKey], output];
}

function spawnParticle(id: string, srcKey: 'ssh' | 'ueba' | 'network' | 'hdfs', output: OutputKey): Particle {
...
export function NetworkCanvas({ alerts, height = 430 }: NetworkCanvasProps) {
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      ...
      if (s.prevCount === 0 && now - s.autoSpawnTime > 1400) {
        s.autoSpawnTime = now;
        const srcs = ['ssh', 'ueba', 'network', 'hdfs'] as const;
        const srcKey = srcs[Math.floor(now / 1400) % 4];
  ...
  function handleClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    const srcKey = hit as 'ssh' | 'ueba' | 'network' | 'hdfs';
